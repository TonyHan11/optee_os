...
//	COMPILE_TIME_ASSERT(PARENT_SIZE >= 11);
	...
	for (i = 0; i < ARRAY_SIZE(sama7_utmick); i++) {
		if (strcmp("utmick", sama7_utmick[i].p) == 0)
			parents[0] = clk;
		else if (strcmp("utmi1", sama7_utmick[i].p) == 0)
			parents[0] = sama7g5_pmc->chws[PMC_UTMI1].clk;
		else
			panic();
		clk = sama7_utmi_clk_register(sama7_utmick[i].n, parents[0],
					      sama7_utmick[i].id);
		if (!clk)
			panic();
